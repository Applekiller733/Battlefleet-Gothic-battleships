...
class Ship_Grid():
    ...
    def __init__(self):
        ...
        self.available_ships_copy = [5, 4, 3, 2, 1]
    ...
    def total_armament(self):
        """
        Resets available armament and recalculates it, then returns the updated list.
        :return:
        """
        self.available_armament = [0, 0]
        for ship in self.ships:
            if ship.is_dead() == 0:
                self.available_armament[0] += ship.macrocannons
                self.available_armament[1] += ship.lances
        return self.available_armament

    def alive_ships(self):
        """
        Counts number of alive ships left in fleet
        :return: num of alive ships
        """
        cnt = 0
        for ship in self.ships:
            if ship.is_dead() == 0:
    ...
    def set_ship_list(self, corv, destr, frig, cru, bships):
        """
        Sets available ships list
        :param corv: num of corvettes
        :param destr: num of destroyers
        :param frig: num of frigates
        :param cru: num of cruisers
        :param bships: num of battleships
        """
        self.available_ships[0] = corv
        self.available_ships[1] = destr
        self.available_ships[2] = frig
        self.available_ships[3] = cru
        self.available_ships[4] = bships
        self.available_ships_copy = self.available_ships.copy()
    ...
    def direction_destringer(self, direction : str):
        """
        Receives a direction string and then transforms it into a tuple.
    ...
    def armament_decider(self, ship_size):
        """
        Decides which kind of armament a ship has given its size.
        :param ship_size: size of ship
        :return: armament tuple
        """
        macrocannons = 0
        lances = 0
        if ship_size <= 4:
    ...
    def generate_board(self):
        ...
        n_ships = 0
        for num in self.available_ships:
            n_ships += num
        """
    ...
    def draw_hit_on_board(self, row, col, type):
        """
        Draws hit on board based on type
        :param row: row
        :param col: col
    ...
    def is_empty(self, board, row, col):
        ...
        """
        try:
            return min(row, col) >= 0 and (board[row][col] == self.EMPTY or board[row][col] == self.HITEMPTY)
        except IndexError:
            pass
        return False
    ...
    def update_alive(self):
        """
        Updates the ship list by checking if the ships in the fleet are still alive/dead.
        :return:
        """
        for ship in self.ships:
            alive = 0
            for r, c in ship.cells:
                if self.board[r][c] != self.HITSHIP:
                    alive = 1
            if alive == 0:
                ship.death()
